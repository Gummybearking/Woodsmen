...
public class MCTree {
	...
	private void completeTree(Block source) {
		...
		case 0:
			...
			for(Object o : trunk.toArray()) {
				completeTreeLeaves (source);
			}
			break;
		case 1:
			type = "redwood";
	...
	private void completeTreeLeaves(Block source) {
		for(Block b : getHorizontalAdjacentBlocks(source)) {
			if(b.getType()==Material.LOG && !trunk.contains(b)) {
				trunk.add(b);
			}
		}
